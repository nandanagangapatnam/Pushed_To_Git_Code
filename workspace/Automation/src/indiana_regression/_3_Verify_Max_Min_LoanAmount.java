...
public class _3_Verify_Max_Min_LoanAmount {
	...
	public void  _2_teller_login () throws Exception{
		...
		xlData = Utils.readXL(OR.getProperty("ipath"),"TestData_IN");
			...
			xlUsername = xlData [35][0];
			xlPassword = xlData [35][1];
			xlStore = xlData [35][2];
	...
	@Test (priority = 4)
	public void  _4_PRA_creation_MaxDraw() throws Exception {
		
		xlResults = Utils.readXL(OR.getProperty("iwpath"),"IN_EPP");
		
		String loanamount = xlData[35][15];
		...
		new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).sendKeys(xlData[35][4]);
		...
		xlResults[8][1] = xlData[35][4];
		xlResults[8][2] = xlData[35][5];
		...
		Thread.sleep(1500);
			if(driver.findElements(By.xpath(xpath)).size() != 0){
				...
				Error_Msg =new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).getText(); // Click on Next button
					...
					if(Error_Msg.contains("The loan amount "+"$"+xlData[35][15]+" is greater than the maximum allowed of $605.00 based on the loan settings")){
						...
						new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).sendKeys(Keys.BACK_SPACE);
						Thread.sleep(1000);
						new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).sendKeys(xlData[35][14]);
						...
						xlResults[8][9] = Error_Msg;	
						xlResults[8][8] = "YIPEE : PASSED";
					}
	...
	public void  _5_PRA_creation_MinDraw() throws Exception {
		...
		String loanamount = xlData[35][13];

		xlResults[9][1] = xlData[35][4];
		xlResults[9][2] = xlData[35][5];	
		...
		if(driver.findElements(By.xpath(xpath)).size() != 0){
			...
			Error_Msg =new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).getText(); // Click on Next button
				...
				if(Error_Msg.contains("The loan amount "+"$"+xlData[35][13]+" is less than the minimum allowed of $50.00 based on the loan settings")){
						...
						new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).sendKeys(Keys.BACK_SPACE);
						Thread.sleep(1000);	
						new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath))).sendKeys(xlData[35][12]);
